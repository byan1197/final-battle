 ...
 public Player(double x, double y, Game game) {
  ...
  if (Game.ChoiceP1 == Game.CHOICEP1.SAADP1) {
   ...
   playerSpecial = new Animation (
     3, tex.saad_special[0], tex.saad_special[1], tex.saad_special[2], tex.saad_special[3], tex.saad_special[4], tex.saad_special[5], tex.saad_special[6],
     tex.saad_special[7], tex.saad_special[8]
     );
   playerSpecialLeft= new Animation (
     3, tex.saad_special[9], tex.saad_special[10], tex.saad_special[11], tex.saad_special[12], tex.saad_special[13], tex.saad_special[14], tex.saad_special[15],
...
/*  if (Physics.Collision(this, game.eb)){
 ...
 public void render(Graphics g){
  ...
  if (jumping && facing == 1) {
   ...
   else if (kick && facing == 1 && !playerKick.playedOnce){
	   playerKick.drawAnimation(g, (int)x, (int)y);
   }
   else if (kick && facing == -1 && !playerKickLeft.playedOnce){
	   playerKickLeft.drawAnimation(g, (int)x, (int)y);
	   
   }
   else if (punch && facing == 1 && !playerPunch.playedOnce){
    playerPunch.drawAnimation(g, (int)x, (int)y);
   }
   else if (punch && facing == -1 && !playerPunchLeft.playedOnce){
    playerPunchLeft.drawAnimation(g, (int)x, (int)y);
   }
   else if (special && facing == 1 && !playerSpecial.playedOnce){
    playerSpecial.drawAnimation(g, (int)x, (int)y);
   }
   else if (special && facing == -1 && !playerSpecialLeft.playedOnce){
    playerSpecialLeft.drawAnimation(g, (int)x, (int)y);
   }
   else {
    if (facing == -1) {
 ...
 public double getY(){
  return y;
	...
	public Rectangle getBounds(){
		return new Rectangle ((int)x+10, (int)y, 130, 225);
	}
